...
export const productsListBadgeSchema = v.pipe(
	v.object({
		label: v.pipe(v.string(), v.description("Text label of the badge")),
		textColor: v.pipe(v.number(), v.description("Text color code")),
		backgroundColor: v.pipe(
			nullishToUndefined(v.number()),
			v.description("Background color code"),
		),
	}),
	v.description("Schema for a product badge."),
);
export const productsListStockAvailabilityTypeSchema = v.pipe(
	v.literal("stock"),
	v.description("Type literal for products stock availability type"),
);
export const productsListStockAvailabilitySchema = v.pipe(
	v.object({
		type: v.pipe(
			productsListStockAvailabilityTypeSchema,
...
export const productsListItemSchema = v.pipe(
	v.object({
		...
		stockAvailability: v.pipe(
			nullishToUndefined(productsListStockAvailabilitySchema),
		...
		badge: v.pipe(
			nullishToUndefined(productsListBadgeSchema),
			v.description("Badge information for the product"),
		),
	...
	v.description("Product card returned by listing endpoints."),
);
export const productsListResponse200Schema = v.pipe(
	v.object({
		filters: v.pipe(
		...
		items: v.pipe(
			v.array(productsListItemSchema),
